...
class FixedSizeBuffer { 
  ...
  add(el) {
    if ((this.buffer.length + 1) > this.size) {
      throw Error('Failed to add element to action repeat buffer: Buffer length exceeded.')
    }
...
function downscaleFrame(imageHeight, imageWidth, callback) {
  let downscaledImage = new Image();


  downscaledImage.onload = function() {

    let downscaledCanvas = document.createElement('canvas');
    let downscaledCtx = downscaledCanvas.getContext('2d');
    
    downscaledCanvas.height = imageHeight;
    downscaledCanvas.width = imageWidth;

    downscaledCtx.drawImage(this, 0, 0, imageWidth, imageHeight);
    

    let imageData = downscaledCtx.getImageData(0, 0, imageWidth, imageHeight);
    let downscaled = '';
  
    for (let i = 0; i < imageData.data.length; i += 4) {
      const r = String.fromCharCode(Math.round(imageData.data[i]));
      const g = String.fromCharCode(Math.round(imageData.data[i + 1]));
      const b = String.fromCharCode(Math.round(imageData.data[i + 2]));
      downscaled = downscaled + r + g + b;
    }

    callback(btoa(downscaled));
  }

  downscaledImage.src = canvas.toDataURL();  
}

const DOWNSCALED_IMAGE_HEIGHT_IN_PIXELS = 84;
...
function sendFrameData(frame, canvas) {
  ...
  if (isTerminal) {
    const payload = {
      terminal: isTerminal,
      buffer: [metadata],
      action: [keySlower, keyFaster, keyLeft, keyRight],
  ...
  if (metadataBuffer.isFull()) {
    const payload = {
      terminal: isTerminal,
      buffer: metadataBuffer.buffer,
      action: [keySlower, keyFaster, keyLeft, keyRight],
      is_initial_frame: IS_INITIAL_FRAME
    };
...
var COLORS = {
  ...
  TREE: '#005108',
  FOG:  '#005108',
  LIGHT:  { road: '#A3A2A2', grass: '#10AA10', rumble: '#555555', lane: '#CCCCCC'  },
  DARK:   { road: '#A8A8A8', grass: '#009A00', rumble: '#BBBBBB'                   },
